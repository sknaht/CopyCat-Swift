...
- (void)updateNavigationBarAndToolBar {
    ...
    self.title = [NSString stringWithFormat:@"%lu/%lu",(unsigned long)(self.currentIndex + 1),(unsigned long)totalNumber];
    BOOL isSeleted = NO;
    ...
    if (self.isFullImage) {
        ...
        NSInteger size = (int)(asset.defaultRepresentation.size/(long long)1024);
        ...
        {
            imageSizeString = [NSString stringWithFormat:@"(%ldK)",(long)size];
        }
...
- (void)updateSelestedNumber
{
    ...
    self.sendButton.badgeValue = [NSString stringWithFormat:@"%lu",(unsigned long)selectedNumber];
}
